const zhCN = {
  ...
  fileRootAccessDescription: '请输入 sudo 目标用户和密码。默认目标用户是 root；如果你的提权目标不是 root，可以手动改成其他用户。若当前主机未配置免密 sudo，请直接输入密码。',
  ...
  fileRootAccessPasswordHintDetail: '如果这台机器已经给该 sudo 用户开了免密 sudo，密码可以留空。没有的话直接输入密码就行；就算你刚在终端里执行过 `sudo -i`，这里也可能还是会单独再确认一次。',
