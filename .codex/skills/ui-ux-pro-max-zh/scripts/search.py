...
if __name__ == "__main__":
    ...
    if args.design_system:
        ...
        if args.persist:
            print("\n" + "=" * 60)
            print(f"✅ 设计系统已成功持久化至 design-system/")
            print(f"   📄 design-system/MASTER.md (全局单一事实来源)")
            if args.page:
                page_filename = args.page.lower().replace(' ', '-')
                print(f"   📄 design-system/pages/{page_filename}.md (页面级覆盖)")
            print("")
            print(f"📖 使用：在构建特定页面时，先检查 design-system/pages/[page].md。")
